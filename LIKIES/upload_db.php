        ...
        if(count($errors)==0){
            ...
            if(empty($e_img) and empty($Edetail)){
                $have = "INSERT INTO record ( U_ID, E_ID, E_date, E_detail, E_pic, E_rating )
                        VALUES ((SELECT U_ID from member WHERE username = '$Uname') , '$last_eid', '$Edate', NULL, NULL, '$Erate' )";
            }
            else if(empty($Edetail)){
                $have = "INSERT INTO record ( U_ID, E_ID, E_date, E_detail, E_pic, E_rating )
                        VALUES ((SELECT U_ID from member WHERE username = '$Uname') , '$last_eid', '$Edate', NULL, '$e_img', '$Erate' )";
            }
            else if(empty($e_img)){
                $have = "INSERT INTO record ( U_ID, E_ID, E_date, E_detail, E_pic, E_rating )
                        VALUES ((SELECT U_ID from member WHERE username = '$Uname') , '$last_eid', '$Edate', '$Edetail', NULL, '$Erate' )";
            }
            else{
                $have = "INSERT INTO record ( U_ID, E_ID, E_date, E_detail, E_pic, E_rating )
                        VALUES ((SELECT U_ID from member WHERE username = '$Uname') , '$last_eid', '$Edate', '$Edetail', '$e_img', '$Erate' )";
            }
            
            if(mysqli_query($conn, $have)){
                header("location: upload.php?remark=success");
            }
